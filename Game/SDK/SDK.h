...
#include "CS2/globalvars.h"
#include "CS2/entity_system.h"
#include "CS2/base_entity.h"



class CSDK : public CSingleton<CSDK>
{
public:
	CSDK();
	bool IsInGame();
	CGlobalVarsBase GetGlobalVars();
	bool WorldToScreen( Vector3 position, Vector3& out, D3DMATRIX viewMatrix );
	Vector2 GetViewAngles();
	bool IsGrenade( uint32_t weaponIndex );
	std::string TranslateGrenade(uint32_t weaponIndex);
...
#define m_pSDK CSDK:GetInstance()

enum WeaponIDI : uint32_t {
	WEAPON_DEAGLE = 1,
	WEAPON_ELITE,
	WEAPON_FIVESEVEN,
	WEAPON_GLOCK,
